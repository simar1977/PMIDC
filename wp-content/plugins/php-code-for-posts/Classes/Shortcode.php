...
class PhpCodeForPosts_Shortcode {
	...
	public static function handle_shortcode( $attributes )
	{
		...
		$arguments = array( 'snippet' => 0, 'slug' => '', 'param' => '' );
		$attributes = shortcode_atts( $arguments, $attributes );

		$snippet_id = intval( $attributes['snippet'] );
		if( $snippet_id == 0 && $attributes['slug'] == '') {
			return '';
		}

		if ($snippet_id > 0) {
			$snippet = PhpCodeForPosts_Database::load_single_snippet( $snippet_id );
		} elseif ($attributes['slug'] != '') {
			$snippet = PhpCodeForPosts_Database::load_single_snippet_by_slug( $attributes['slug'] );
		} else {
			$snippet = false;
		}

		if (! $snippet || ! $snippet->is_loaded() ) {
			return '';
		}
	...
	{
		$arguments = array( 'snippet' => 0, 'slug' => '', 'param' => '', 'blog_id' => 1 );
		$attributes = shortcode_atts( $arguments, $attributes );
		...
		if ($snippet_id > 0) {
			...
			$snippet = PhpCodeForPosts_Database::load_single_snippet_by_slug( $attributes['slug'], $blog_id );
		} else {
			$snippet = false;
		}
