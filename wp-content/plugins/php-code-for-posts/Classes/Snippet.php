...
class PhpCodeForPosts_Snippet {
	...
	public function get_display_shortcode($useSlug = false)
	{
		if ($useSlug) {
			return sprintf( '[%s slug=%s]', PhpCodeForPosts_Shortcode::get_shortcode(), $this->get_slug() );
		}
		return sprintf( '[%s snippet=%s]', PhpCodeForPosts_Shortcode::get_shortcode(), $this->get_id() );
	}

	public function get_multisite_display_shortcode($useSlug = false)
	{
		if ($useSlug) {
			return sprintf( '[%s slug=%s]', PhpCodeForPosts_Shortcode::get_multisite_shortcode(), $this->get_id() );
		}
	...
	public static function save_posted_snippet( $post_fields )
	{
		...
		$snippet->set_slug( $post_fields['slug'] );
