...
class adswsc_clsWidget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		$instance['bosize'] = empty($instance['bosize']) ? '' : $instance['bosize'];
		$instance['botype'] = empty($instance['botype']) ? '' : $instance['botype'];
		$instance['bocolor'] = empty($instance['bocolor']) ? '' : $instance['bocolor'];
		$instance['boradius'] = empty($instance['boradius']) ? '' : $instance['boradius'];
		...
		$options['bosize'] = apply_filters( 'widget_bosize', $instance['bosize'] );
		$options['botype'] = apply_filters( 'widget_botype', $instance['botype'] );
		$options['bocolor'] = apply_filters( 'widget_bocolor', $instance['bocolor'] );
		$options['boradius'] = apply_filters( 'widget_boradius', $instance['boradius'] );
    ...
    $bosize = isset( $instance[ 'bosize' ] ) ? $instance[ 'bosize' ] : 3;
		$botype = isset( $instance[ 'botype' ] ) ? $instance[ 'botype' ] : 'inset';
		$bocolor = isset( $instance[ 'bocolor' ] ) ? $instance[ 'bocolor' ] : 'white';
		$boradius = isset( $instance[ 'boradius' ] ) ? $instance[ 'boradius' ] : 3;
		...
		echo '<input type="text" maxlength="200" class="widefat" id="'.$this->get_field_id( 'title' ).'" name="'.$this->get_field_name( 'title' ).'" value="'.esc_attr( $title ).'" />';
		echo '</p>';
		
		// before
		echo '<p>';
		echo '<label for="'.$this->get_field_id( 'before' ).'">'.__('Text before counter', ADS_TEXT_DOMAIN).'</label><br>';
		echo '<input type="text" maxlength="200" class="widefat" id="'.$this->get_field_id( 'before' ).'" name="'.$this->get_field_name( 'before' ).'" value="'.esc_attr( $before ).'" />';
		echo '</p>';

		// after
		echo '<p>';
		echo '<label for="'.$this->get_field_id( 'before' ).'">'.__('Text after counter', ADS_TEXT_DOMAIN).'</label><br>';
		echo '<input type="text" maxlength="200" class="widefat" id="'.$this->get_field_id( 'after' ).'" name="'.$this->get_field_name( 'after' ).'" value="'.esc_attr( $after ).'" />';
		echo '</p>';
		...
		// border
		echo '<p>';
		echo '<label for="'.$this->get_field_id( 'bosize' ).'">'.__('Image-border size(px) / type / #color / radius(px)', ADS_TEXT_DOMAIN).'</label><br>';
		echo '<span style="vertical-align:middle; white-space:nowrap;">';
		echo '<input style="width:20%;" type="number" maxlength="1" min="0" max="9" class="widefat" id="'.$this->get_field_id( 'bosize' ).'" name="'.$this->get_field_name( 'bosize' ).'" value="'.min(9,max(0, absint( $bosize ))).'"/>&nbsp';
		echo '<select id="'.$this->get_field_id( 'botype' ).'" name="'.$this->get_field_name( 'botype' ).'" >';
		echo '<option value="inset" '.($botype == 'inset' ? "selected" : "" ).' >inset</option>';
		echo '<option value="outset" '.($botype == 'outset' ? "selected" : "" ).' >outset</option>';
		echo '<option value="solid" '.($botype == 'solid' ? "selected" : "" ).' >solid</option>';
		echo '<option value="dotted" '.($botype == 'dotted' ? "selected" : "" ).' >dotted</option>';
		echo '<option value="double" '.($botype == 'double' ? "selected" : "" ).' >double</option>';
		echo '<option value="hidden" '.($botype == 'hidden' ? "selected" : "" ).' >hidden</option>';
		echo '<option value="ridge" '.($botype == 'ridge' ? "selected" : "" ).' >ridge</option>';
		echo '<option value="initial" '.($botype == 'initial' ? "selected" : "" ).' >initial</option>';
		echo '<option value="inherit" '.($botype == 'inherit' ? "selected" : "" ).' >inherit</option>';
		echo '<option value="none" '.($botype == 'none' ? "selected" : "" ).' >none</option>';
		echo '</select>';
		echo '<input style="width:20%;" type="text"  class="widefat" id="'.$this->get_field_id( 'bocolor' ).'" name="'.$this->get_field_name( 'bocolor' ).'" value="'.esc_attr( $bocolor ).'" />&nbsp';
		echo '<input style="width:20%;" type="number" maxlength="1" min="0" max="9" class="widefat" id="'.$this->get_field_id( 'boradius' ).'" name="'.$this->get_field_name( 'boradius' ).'" value="'.min(9,max(0, absint( $boradius ))).'"/>&nbsp';
		echo '</span>';
		echo '</p>';
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['bosize'] = ( ! empty( $new_instance['bosize'] ) ) ? strip_tags(min(9,max(0, absint( $new_instance['bosize'] ) ))) : '';
		$instance['botype'] = ( ! empty( $new_instance['botype'] ) ) ? strip_tags( $new_instance['botype'] ) : '';
		$instance['bocolor'] = ( ! empty( $new_instance['bocolor'] ) ) ? strip_tags( $new_instance['bocolor'] ) : '';
		$instance['boradius'] = ( ! empty( $new_instance['boradius'] ) ) ? strip_tags(  min(9,max(0, absint( $new_instance['boradius'] ) ))) : '';
		return $instance;
	}
} 
