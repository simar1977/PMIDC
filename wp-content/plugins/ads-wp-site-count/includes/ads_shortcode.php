...
class adswsc_clsShortCode  {
	...
	public function post_handler($args) {
		...
		$args = shortcode_atts( array (
			...
			'reset' => '',
			'save' => '',
			'bosize' => $options['bosize'],
			'botype' => $options['botype'],
			'bocolor' => $options['bocolor'],
			'boradius' => $options['boradius']
		), $args);
	...
	public function post_function($args) {
		...
		$args['bosize'] = absint($args['bosize']);
		$args['botype'] = wp_specialchars_decode($args['botype']); 
		$args['botype'] = (in_array( $args['botype'], 
			array( 'inset','outset','solid','dotted','double','hidden','ridge','initial','inherit','none' ) ) ? $args['botype'] : 'inset' );
		$args['bocolor'] = wp_specialchars_decode($args['bocolor']); 
		$args['boradius'] = absint($args['boradius']); 
		if ($args['reset'])
