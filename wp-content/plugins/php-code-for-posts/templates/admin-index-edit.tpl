<div class='tabs phpcfp-tabs'>
	<div class='tab tab-snippets activetab'>
		...
		<form action="?page=<?php echo PhpCodeForPosts_Menu::MENU_SLUG?>" method="post" id="codeform" <?php if ($snippet->get_id() && PhpCodeforPosts::$options->get_option('ajaxible') == 1) { echo 'class="ajaxible"'; }?>>
			...
			<br />
			<label for="<?php echo PhpCodeForPosts::__input_id( 'slug' ) ?>"><strong><?php _e('Code slug', "phpcodeforposts" ) ?></strong> <span style="color:red; font-weight:bold">Beta!</span></label>
			<input class="widefat" id="<?php echo PhpCodeForPosts::__input_id( 'slug' ) ?>" maxlength="256" name="<?php echo PhpCodeForPosts::__input_name( 'slug' ) ?>" placeholder="<?php _e( 'Unique slug to add your snippet via your shortcode', "phpcodeforposts" ) ?>" type="text" value="<?php echo esc_attr( $snippet->get_slug() ) ?>" />
			<br />&nbsp;
